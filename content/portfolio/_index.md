+++
date = '2026-04-22T00:00:00-04:00'
draft = false
title = 'Portfolio'
description = 'Projects built from scratch and documented in real time.'
author = 'Jesse Bentley'
showToc = false
hideMeta = true
+++

A collection of projects built from scratch and documented honestly on this blog. Every line of code written, every mistake made, and every problem solved is recorded in the corresponding blog series.

---

## Active Projects

<details>
<summary><strong>Flipper Zero Physical Security Suite</strong></summary>

A complete hardware security toolkit programmed from scratch in C, C++, and Python. No pre-built firmware — every feature written and documented line by line.

**Hardware:**
→ Flipper Zero (main device)
→ ESP32 WiFi Dev Board
→ ESP32-CAM with 940nm IR LEDs
→ NRF24L01+PA+LNA 2.4GHz module
→ NEO-6M GPS module
→ Elecrow CrowPanel 3.5" LVGL touchscreen
→ Custom 3D printed enclosure (PETG)

**Features being built:**
→ WiFi device scanner (hidden camera detection)
→ Sub-GHz RF analyzer
→ IR lens detector
→ NFC/RFID reader
→ GPS location logging
→ LVGL touchscreen dashboard
→ Python companion app

**Blog series:** [Flipper Zero Build](/series/flipper-zero-build/)

**GitHub:** Coming soon

</details>

---

<details>
<summary><strong>GameMaster AI</strong></summary>

A reinforcement learning system that teaches AI agents to play Steam games from scratch. No Gymnasium, no Stable Baselines — everything built by hand in Python and PyTorch.

**Tech stack:**
→ Python 3.11
→ PyTorch (custom neural networks)
→ Tesseract OCR (screen state extraction)
→ Anthropic API (LLM program synthesis)
→ NVIDIA RTX 3070 Ti (training GPU)

**Current status:**
→ Phase 1 complete — Melvor Idle
  training with 1,102 auto-discovered
  actions
→ Phase 2 starting — Military
  Incremental Complex

**Transfer learning experiment:**
→ Melvor Idle → Stardew Valley
→ Measuring episodes to competence
→ Testing negative transfer effects

**Blog series:** [GameMaster AI](/series/gamemaster-ai/)

**GitHub:** Coming soon

</details>

---

## Planned Projects

## Conference Talks
Target 2027:

→ BSides Knoxville

→ BSides Nashville

→ HackSpace Con

→ Hack Red Con

→ BSides Charleston

## Open Source

Everything built on this blog is
documented publicly on GitHub.
The goal is to contribute something
back to the community that's been
generous with knowledge and mentorship.

→ [GitHub](https://github.com/JDBentley)