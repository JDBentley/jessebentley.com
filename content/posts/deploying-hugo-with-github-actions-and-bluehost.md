...
Outside of that everything went slower than I anticipated — but that was intentional. I took my time with every step, made sure I understood what each command was doing before running it, and it paid off. The GitHub Actions workflow file, the SSH key generation, wiring the secrets into the repo — none of it was as complicated as it looked at first glance. It just required patience and reading carefully.
...
Watching the GitHub Actions pipeline run for the first time and seeing the site update live was genuinely satisfying. Push to GitHub, watch the workflow tab, site updates in under 90 seconds. Every future blog post works the same way — write it, commit it, push it, done.
